...
struct TravelInfo getTravelInfo() {
    ...
    // Query API
    httpClient.begin(String(F("https://api.torn.com/user/?selections=travel,basic,bars&key=")) + API_KEY, SSL_FINGERPRINT);
    ...
    if (httpCode > 0) {   // http err if negative code
        ...
        const size_t capacity = JSON_OBJECT_SIZE(4) +
                                ...
                                JSON_OBJECT_SIZE(12) + 510;   // generated by ArduinoJson "assistant"
        DynamicJsonDocument doc(capacity);
        DeserializationError err = deserializeJson(doc, payload);

        // Return JSON error to be printed
        if (err) {
            TravelInfo ret = {"", String("JSON err: ") + err.c_str(), 0, 0, 0, 0, 0};
            return ret;
        }
        ...
        TravelInfo ret = {
            ...
            doc["energy"]["current"].as<uint16_t>(),
            doc["energy"]["maximum"].as<uint16_t>(),
            doc["nerve"]["current"].as<uint16_t>(),
            doc["nerve"]["maximum"].as<uint16_t>()
        };
        return ret;
    }
    ...
    // Return HTTP error to be printed
        ...
        TravelInfo ret = {"", String("HTTP error: ") + httpClient.errorToString(httpCode).c_str(), 0, 0, 0, 0, 0};
        return ret;
    }
}
